...
namespace AssetManagement_DataAccess
{
    public class SQL_DB
    {
        ...
        //execute procedure to return datatable 
        ...
        public async Task<ProcedureResult> ExecuteProcedureWithOutputAsync(string procedureName, Dictionary<string, object>? parameters = null, string? outputParamName = null)
        {
            using (var conn = new SqlConnection(_ConnectionString))
            {
                try
                {
                    ...
                    using (var cmd = new SqlCommand(procedureName, conn))
                    {
                        ...
                        {
                            outputParam = new SqlParameter(outputParamName, SqlDbType.VarChar, 500)
                            {
                        ...
                        using (var reader = await cmd.ExecuteReaderAsync())
                        {
                            ...
                            {
                                Data = dataTable,
                                OutputValue = (outputParam != null && outputParam.Value != DBNull.Value) ? outputParam.Value?.ToString() : null
                            };
                        }
                    }
                }
        ...
        // method to execute that procedure which don't return datatable 
        public async Task<int> ExecuteProcedureNonQueryAsync(string procedureName, Dictionary<string, object> parameters = null)
        {
            using (var conn = new SqlConnection(_ConnectionString))
            {
                try
        ...
        /// <summary>
        /// Performs bulk insert into SQL Server table.
        /// </summary>
        /// <param name="dataTable">Source DataTable</param>
        /// <param name="tableName">Destination SQL table name</param>
        /// <param name="columns">Array of column names (DataTable → DB Table)</param>
        /// <summary>
        /// Performs bulk insert into SQL Server table.
        /// </summary>
        /// <param name="dataTable">Source DataTable</param>
        /// <param name="tableName">Destination SQL table name</param>
        /// <param name="columns">Array of column names (DataTable → DB Table)</param>
        public async Task<bool> BulkInsertAsync(DataTable dataTable, string tableName, string[] columns)
        {
            if (dataTable == null || dataTable.Rows.Count == 0 || columns == null || columns.Length == 0)
                return false;
            ...
            {
                ExceptionLogs($"SQL Error: {sqlEx.Message}, Table: {tableName}, StackTrace: {sqlEx.StackTrace}");
                return false;
            }
            catch (Exception ex)
            {
                ExceptionLogs($"General Error: {ex.Message}, Table: {tableName}, StackTrace: {ex.StackTrace})");
                return false;
            }
        }
    }
