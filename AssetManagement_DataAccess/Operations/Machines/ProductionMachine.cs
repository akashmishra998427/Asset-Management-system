...
namespace AssetManagement_DataAccess
{
    public class ProductionMachine
    {
        ...
        //        returnVaue = username.ToString();
        //        var ret = returnVaue.Split('*');
        //        if (ret.Length > 1)
        //        {
        //            CreatebarCode(ret[1]);
        //        }
        ...
        public async Task<string> ManageExcelData(List<ProductionMachineEntity> manageExcelData, string status, string username)
        {
            try
            {
                ...
                var param = new Dictionary<string, object>
                {
                    ...
                    { "@Data", string.IsNullOrEmpty(status) ? "" : status },
                    { "@UserName", username ?? "" },
                    { "@GetExcelTempValue", excelData }
                };

                // Execute SP
                var result = await _SQL_DB.ExecuteProcedureWithOutputAsync(
                    "Ol_Proc_MachineData",
                    param,
                    "ReturnValue"
                );

                string message = result.OutputValue ?? "";

                // Barcode Generation
                var parts = message.Split('*');
                if (parts.Length > 1)
                {
                    CreatebarCode(parts[1]);
                }
            ...
            {
                _SQL_DB.ExceptionLogs(
                    $"Error in ManageExcelData: {ex.Message}, StackTrace: {ex.StackTrace}"
                );
                return "An error occurred while processing the request.";
            }
        }
        ...
        public async void CreatebarCode(string MachineCode)
        {

            string strbind = "IF  NOT EXISTS(SELECT * FROM ol_MachineDetail WHERE MachineCode IN (SELECT item FROM DBO.SPLIT('" + MachineCode + "',',')) ) INSERT INTO ol_MachineDetail(MachineCode) SELECT item FROM DBO.SPLIT('" + MachineCode + "',',') ";
            await _SQL_DB.ExecuteQuerySelect(strbind);

            DataTable dt = new DataTable();
            strbind = " SELECT M.id, D.MachineSrNo, D.MachineCode FROM ol_MachineDetails D INNER JOIN ol_MachineDetail  M on D.MachineCode = M.MachineCode where D.MachineCode IN (SELECT item FROM DBO.SPLIT('" + MachineCode + "',',')) ";
            dt = await _SQL_DB.ExecuteQuerySelect(strbind);
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dtrow in dt.Rows)
                {
                    string code = "";
                    code = "Code : " + dtrow["MachineCode"].ToString();
                    code += ",Sno : " + dtrow["MachineSrNo"].ToString();
                    ...
                    using (Bitmap bitMap = qrCode.GetGraphic(20))
                    {
                        using (MemoryStream ms = new MemoryStream())
                        {
                            ...
                            string sql = $"UPDATE ol_MachineDetail SET Barcode =  '{byteImage}' WHERE id = '{dtrow["id"].ToString()}'";
                            _SQL_DB.ExecuteNonQuery(sql);
                        }
