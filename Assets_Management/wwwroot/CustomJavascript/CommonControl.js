    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            sums.forEach((sum, index) => {
                if (index > 0) {
                    footerRow += `<td onclick="FooterDetail(this)">${sum.toFixed(0)}</td>`;
                }
            });
            footerRow += `</tr>`;
        ...
        success: function (data) {
            try {
                ...
                if (data && data.success === false) {
                    $(`#${tblBody}`).empty().append(`<tr><td colspan="100%" class="text-center">${data.message || 'No data found'}</td></tr>`);
                    return;
                }

                if (!Array.isArray(data)) {
                    $(`#${tblBody}`).empty().append('<tr><td colspan="100%" class="text-center">Invalid data format</td></tr>');
                    return;
                }

                if (data.length === 0) {
                    $(`#${tblBody}`).empty().append('<tr><td colspan="100%" class="text-center">No data available</td></tr>');
                    return;
                }

                const firstItem = data[0];
                if (!firstItem || typeof firstItem !== 'object') {
                    $(`#${tblBody}`).empty().append('<tr><td colspan="100%" class="text-center">Invalid data structure</td></tr>');
                    return;
                }

                let headers = Object.keys(firstItem);
                if (headers.length === 0) {
                    $(`#${tblBody}`).empty().append('<tr><td colspan="100%" class="text-center">No data structure available</td></tr>');
                    return;
                }//a
                ...
                console.error("Failed to parse or render data:", e);
                $(`#${tblBody}`).empty().append('<tr><td colspan="100%" class="text-center">Error processing data</td></tr>');
            }
        },
        error: function (xhr, status, error) {
            console.error("Error fetching data:", error);
            console.error("Response:", xhr.responseText);
            $(`#${tblBody}`).empty().append('<tr><td colspan="100%" class="text-center">Error loading data. Please try again.</td></tr>');
        },
        complete: function () {
        }
    });
...
function loadCheckboxesWithSearch2(apiUrl, labelField, valueField, sectionId, className) {
    ...
    function renderSelectAllCheckbox() {
        ...
        $(`#${sectionId}`).empty().append($selectAllLabel);
    ...
    function renderCheckboxes(items) {
        $(`#${sectionId}`).find('label').not(':first-child').remove();
        $(`#${sectionId}`).find('p').remove();
        const processedItems = items.map(item => ({
            value: item[valueField],
            label: item[labelField]
        })).filter(item => item.value !== undefined && item.label !== undefined);
        ...
        processedItems.forEach(item => {
            ...
            const $checkbox = $(`<input type="checkbox" class="${className || ''}" data-individual="true">`).val(item.value).prop('checked', isChecked);
            $label.append($checkbox).append(' ' + item.label);
            if (isChecked) {
                checkedItems.push({ label: item.label, $el: $label });
            } else {
    ...
    function updateSelectAllState() {
        if (selectAllCheckbox) {
            ...
            const selectedCount = fullData.filter(item =>
                selectedValues.has(item[valueField])
            ).length;
            if (selectedCount === 0) {
...
function loadCheckboxesWithSearchChecked(apiUrl, labelField, valueField, sectionId, className, preSelectedValues = []) {
    ...
    function renderSelectAllCheckbox() {
        $(`#${sectionId} .select-all-wrapper`).remove();
        const $selectAllLabel = $('<label class="select-all-wrapper text-dark" style="display:block; font-weight:bold; padding-bottom:5px; margin-bottom:10px;"></label>');
        selectAllCheckbox = $(`<input type="checkbox" class="${className || ''}" id="selectAll_${sectionId}">`);
        selectAllCheckbox.on('focus', function () {
            $(this).blur();
        });
        $selectAllLabel.append(selectAllCheckbox).append(' Select All');
        $(`#${sectionId}`).empty().append($selectAllLabel);
        selectAllCheckbox.on('change', function () {
            ...
            fullData.forEach(item => {
                if (isChecked) {
                    ...
                    selectedValues.delete(item[valueField]);
                }
            });
            renderCheckboxes(fullData);
        });
    ...
    function renderCheckboxes(items) {
        $(`#${sectionId}`).children('label').not('.select-all-wrapper').remove();
    ...
    function updateSelectAllState() {
        if (selectAllCheckbox) {
            const totalItems = fullData.length;
            const selectedCount = fullData.filter(item => selectedValues.has(item[valueField])).length;
            if (selectedCount === 0) {
...
function attachDynamicSuggestion(controlId, apiUrl, Type) {
   
    const $input = $(`#${controlId}`);
     
    if ($input.length === 0) {
        console.warn(`Input field #${controlId} not found. Skipping suggestion attachment.`);
        return;
    }

    let suggestionData = [];
    let $suggestionBox = $(`#${controlId}_suggestions`);
     
    if ($suggestionBox.length > 0) {
        $suggestionBox.remove();
    }
     
    $suggestionBox = $(`<div id="${controlId}_suggestions"></div>`).css({
        position: 'absolute',
        background: '#fff',
    ...
    async function fetchSuggestions() {
        try {
            ...
            suggestionData = typeof response === 'string' ? JSON.parse(response) : response;
    ...
    function filterSuggestions(query) {
        const filtered = suggestionData.filter(item => {
            const text = item?.DataValue || item?.toString() || '';
    ...
    $(document).on('click', function (e) {
        if (!$(e.target).closest($input).length &&
            !$(e.target).closest($suggestionBox).length) {
            $suggestionBox.hide();
        }
    });
